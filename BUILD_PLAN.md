...
Phase 5 note. Migration `0003_workspace_scoping.sql` adds `workspace_id uuid` to `sheets` (backfilled from each sheet's document) and indexes `workspace_id` on `documents` and `sheets`, so a per-visitor read can be scoped and verified directly without joining through the document on every request. `documents` already carried `workspace_id`. Both are written on creation with the requesting visitor's anonymous workspace id. The migration is idempotent.
...
Theme (Phase 5 visual workstream, 2026-06-04). The product reads as a cool blue spreadsheet, with a palette sampled from the owner's sky photo (the sky averages #4F6C9D, the clouds #C8CBD9). Every structural color flows from one source, `apps/web/lib/theme.ts`: Tailwind reads it for the utility-class tokens (paper, surface, ink, muted, faint, line and line-strong, primary and primary-deep and primary-tint, cloud), and the one place that needs raw color strings (the recharts grid and axis chrome) imports the same values, so a restyle is a one-file edit. The two color sets that carry meaning are deliberately not part of this palette and were not touched: the confidence band colors in `lib/sheet/confidence.ts` and the section category accents in `lib/sheet/category.ts` (and their export counterpart `app/export/palette.py`). A gate sweeps the web source for the previous warm hex values to prove the restyle is complete, a gate pins the confidence colors byte for byte, and a gate fixes the sidebar layout. The sidebar carries the signature element: a decorative sky banner (`apps/web/public/brand/sky.jpg`, empty alt, cover crop) directly below the Documents header, with the document list below the banner. The asset is a single static file, so swapping the photo later is a one-file change with no code change.
...
Scope revision and status as of 2026-06-04. The owner reshaped Phase 5 into a demo-grade hardening pass with authentication deliberately deferred, because the product is shared over a public link with a trusted network looking at public research, not real client documents. Authentication returns when real client M&A documents start flowing. Built and green on the deterministic, secret-free gates:

- Per-visitor isolation without login (the headline). Each visitor gets a private space keyed by an anonymous, long, random workspace id generated in the browser and persisted in localStorage; it rides on every request (a header for fetch, the ws query parameter for the events stream and download anchors, which cannot set headers). Migration 0003 tags sheets with workspace_id; documents and sheets are tagged on creation, and every list, sheet read, export, events, and original is scoped and verified by it, returning not found for another visitor's resource. Requests with no id fall back to the default workspace, keeping the seeded data and the Phase 0 to 4 tests unchanged. This is isolation, not authentication: see CLAUDE.md Section 9. Gate: a DB-backed secret-free test proves A's document is invisible to B and a 404 on its sheet, document, export, events, and original by id.
- Rate limiting and a usage backstop on upload and extract, keyed by workspace plus client address, with a clear 429, behind the owner's OpenAI spend cap. Gates: a secret-free limiter unit test and DB-gated endpoint hammer tests.
- Large-document chunk-and-merge. A document over the single-pass budget is split into ordered overlapping chunks; discovery sees a representative head-middle-tail sample; each section is extracted per chunk; every value is grounded against the FULL canonical text so its span is a full-text offset and the Phase 4 in-document highlight still works; the per-chunk results merge with row de-duplication. The chunk cap is a high safety backstop, not a cost control; past it the leading portion is processed and the truncation is reported, never silent. Gate: secret-free chunking unit tests; the live large-document extraction is owner and secret gated.
- Reveal robustness: the unbuffered stream headers and heartbeat plus the client polling fallback (gate-tested) always complete the reveal; the fallback backstop was extended to about 20 minutes so a long large-document run is never a false failure.
- Durable originals: the Supabase Storage backend is wired and selected by STORAGE_BACKEND; the owner provisions the bucket and the service role key. Observability: structured stdout logging with pipeline-failure capture. States: empty, loading, extracting, done, and failed all render intentionally. UI polish: the upload screen and the landing match the warm sheet palette.

Owner-confirmed gates: the safety guards (rotate the OpenAI key and Supabase password, set a spend cap, confirm no secret in git history, which is verified clean) and the live staging demo pass (a clean browser shows an empty private space, a document uploads and extracts and renders, the workbook downloads, a value highlights its source, and a second browser is a separate empty space). Image and chart reading (Phase 2.5) and authentication remain deferred.

PHASE 5 COMPLETE (owner-confirmed, 2026-06-04). The deterministic gates are green in CI on a clean checkout, the staging API is deployed and serving the new build (GET /health reports extraction_prompt_version 3, confirming the prompt-caching cost reduction and the per-visitor isolation are live), migration 0003 is applied to the staging database, originals are on durable Supabase Storage, and the owner has confirmed the safety guards and the live staging pass. This was the final planned phase: the build is end to end and presentable at a demo-grade, no-auth security posture, ready to share with a trusted network over a link. A cost-reduction pass landed on top of Phase 5: the extraction prompt now sends the document as a cached prefix (prompt v3) so OpenAI prompt caching bills the repeated document at the cached rate, with cached-token and real cost_usd tracking; verification can move to a cheaper model via OPENAI_MODEL_VERIFICATION for a further saving. Authentication and the confidentiality work that goes with it return when real client M&A documents start flowing.
